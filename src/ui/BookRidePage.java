    ...
    public static void main(String[] args) {
        ...
        btnFare.addActionListener(e -> {
            try {
                double distance = Double.parseDouble(
                        txtDistance.getText());
                if (distance <= 0) {
                    JOptionPane.showMessageDialog(frame, "Distance must be greater than 0!");
                    return;
                }
        ...
        btnConfirm.addActionListener(e -> {
            try {
                ...
                if (pickup.isEmpty() || destination.isEmpty() || txtDistance.getText().trim().isEmpty()) {
                ...
                double fare = vehicle.calculateFare(distance);
                Ride ride = new Ride(RideManager.generateRideId(), pickup, destination, vehicleType, distance, fare,
                        "Pending");

                ride.setUserEmail(LoginManager.currentUserEmail);
                RideManager manager = new RideManager();
                manager.bookRide(ride);

                JOptionPane.showMessageDialog(frame, "Ride Booked Successfully!\n\n"
                        + "Ride ID : "
                        + ride.getRideId()
                        + "\nFare : "
                        + ride.getFare()
                ...
                fareLabel.setText("Fare : 0.00 BDT");
